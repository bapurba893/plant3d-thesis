"""
train_c2_dgcnn_da_a.py

Strategy table row C2: DGCNN backbone, DA-A (adversarial domain
adaptation), ALL augmentation, L_cls + L_seg + L_dom + L_ent -- the
adversarial anchor row (identical method to A2/A4/B2/C4; only the backbone
or augmentation varies across those). Builds directly on C1
(train_c1_dgcnn_da0.py): same backbone (DGCNN_ClsSeg), same L_cls/L_seg
setup with Kendall uncertainty weighting, same source data/splits/protocol
-- this script adds exactly one variable on top, the adversarial machinery,
per CLAUDE.md's row-ordering note ("isolated, one-variable-at-a-time
increment on top of C1 rather than a rewrite").

What's new vs. C1:
  - Target data (Pheno4D) is now used during training -- but ONLY its
    points, never its labels. Source: data/pheno4d_adaptation_pool.csv
    (160 plants), distinct from data/pheno4d_heldout_eval.csv (63 plants,
    still used purely as a hands-off epoch-by-epoch domain-gap indicator,
    exactly as in C1/A1 -- never trained on, never used for model
    selection).
  - L_dom: a domain discriminator (adapters/dann.py::DomainDiscriminator)
    tries to tell source features from target features; a Gradient
    Reversal Layer (GRL) flips the sign of that gradient on its way back
    into the shared DGCNN backbone, so a single optimizer step both trains
    the discriminator to detect domain and trains the backbone to hide it.
  - L_ent: entropy minimization on the model's own (unlabeled) target
...
only). L_dom/L_ent are intentionally excluded from the selection criterion:
they involve target features/predictions and are adversarial/entropy
terms, not accuracy measures, so a lower L_dom does not mean a better
model, and including them would either require target labels (defeating
the point) or select on an unreliable signal. This also matches
DefRec_and_PCM's own PointDA/trainer.py convention ("save model according
to best source model, since we don't have target labels").
"""
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Row C2: DGCNN, DA-A (adversarial), ALL augmentation, L_cls+L_seg+L_dom+L_ent")
    p.add_argument("--exp_name", type=str, default="C2_dgcnn_da_a")
...
def evaluate_cls_only(model, loader, criterion, device):
    ...
    C1/A1's held-out eval; target segmentation still isn't wired (label
    spaces don't reconcile -- see train_c1_dgcnn_da0.py's docstring)."""
    model.eval()
...
def main():
    ...
    # as source, per the row's DA-A/ALL spec) -- labels are loaded by
    ...
    # L_cls species-level class weights -- same w_c = (f_c+eps)^-1 pattern as C1.
    ...
    n_batches_per_epoch = len(src_train_loader)  # epoch length is source-driven, as in PointDA/trainer.py
