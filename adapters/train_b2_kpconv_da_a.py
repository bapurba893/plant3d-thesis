"""
train_b2_kpconv_da_a.py

Strategy table row B2: KPConv backbone, DA-A (adversarial domain
adaptation, the anchor method), ALL augmentation, L_cls + L_seg + L_dom +
L_ent -- the KPConv-track counterpart of adapters/train_c2_dgcnn_da_a.py
(row C2) and adapters/train_a2_pointnet2_da_a.py (row A2). Mirrors those
scripts' DA-A protocol, same adversarial machinery (adapters/dann.py,
reused byte-for-byte -- already backbone-agnostic, already fixed for the
ramped-lambda_ent bug found during C2) and same Kendall({"cls","seg"})
combination -- only the backbone (adapters/models_kpconv.py::KPConv_ClsSeg,
which already exposes logits["feat"] from B1, no change needed) and the
batch/collate mechanics it requires (see adapters/kpconv_collate.py) differ.

Purpose of this row, per explicit user instruction: B1 (KPConv, DA-0) had
the worst selection-signal reliability of any DA-0 baseline across all
three backbones so far (corr(source val loss, target acc) = +0.316 --
positive, meaning a BETTER source loss predicted WORSE target accuracy,
unlike A1 -0.165 or C1 -0.286 -- see step_notes/B1_KPConv_DA0.md). This row
tests whether that already-poor reliability gets worse under DANN the way
A2/C2 both showed a real (if backbone-dependent-in-magnitude) DA-A
underperformance vs. their own DA-0 baselines, or whether KPConv's distinct
instability signature (DGCNN-like collapse resistance, PointNet++-or-worse
continuous variance) behaves differently once adversarial pressure is
added.

One real, KPConv-specific design decision beyond a straight backbone swap:
`neighborhood_limits` (see kpconv_collate.py::calibrate_neighborhood_limits)
is now calibrated over BOTH domains, not source-only like B1. B1 never
processed target points through the encoder at all (true DA-0), so
source-only calibration was sufficient; DA-A rows push target-domain
batches through the identical encoder every step, and step_notes/
A3_PointNet2_DA_S.md's still-unverified density-sensitivity hypothesis
flags a real, checked (not assumed) ~2-13x raw point-density gap between
Crops3D and Pheno4D -- calibrating on source alone risks under-covering
target-domain neighbor counts, which is exactly the failure mode that
caused B1's original CUDA OOM incident. `kpconv_collate.py` gained
`collate_fn_factory` (so calibration can run against Pheno4D's cls-only
2-tuple dataset, not just Crops3D's cls+seg 3-tuples) and
`combine_neighborhood_limits` (elementwise max across the two domains'
per-layer caps) to do this cheaply and safely.

DomainDiscriminator(in_dim=...): KPConv_ClsSeg's pooled bottleneck feature
(`logits["feat"]`) is 256-wide (PlantKPConvConfig: first_features_dim=64,
doubled twice by two resnetb_strided blocks -> 64->128->256; traced
directly from KPConv_ClsSeg.__init__, not assumed) -- NOT 1024 like
DGCNN/PointNet2's global feature. Using in_dim=1024 here (a naive copy from
A2/C2) would silently mismatch the discriminator's first Linear layer;
using the correct traced value is a real, KPConv-specific detail this row
had to get right, not a copy-paste default.

Model selection: same protocol as every prior row -- best checkpoint by
lowest source validation total loss (Kendall(cls,seg) on Crops3D val only,
never touches target labels). L_dom/L_ent intentionally excluded from that
selection signal, same reasoning as C2/A2.

Per explicit user instruction, gets the same full-trajectory analysis as
every prior DA-A row from the start -- see step_notes/B2_KPConv_DA_A.md.
"""
...
KPCONV_FEAT_DIM = 256  # KPConv_ClsSeg's pooled bottleneck width -- see module docstring
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Row B2: KPConv, DA-A (adversarial), ALL augmentation, L_cls+L_seg+L_dom+L_ent")
    p.add_argument("--exp_name", type=str, default="B2_kpconv_da_a")
    ...
    p.add_argument("--verbose_batches", action="store_true",
                    help="log per-batch collate/step timing, same diagnostic B1 added after "
                         "job 308845's silent stall -- see step_notes/B1_KPConv_DA0.md")
    p.add_argument("--num_workers", type=int, default=0,
                    help="kept at the safe default -- B1 found DataLoader(num_workers>0) made "
                         "things worse (deadlock/severe slowdown) with these compiled C++ "
                         "extensions, see step_notes/B1_KPConv_DA0.md")
    return p.parse_args()
...
def main():
    ...
    # Calibrate neighborhood_limits over BOTH domains, not source-only like B1 -- see module
    # docstring for why (DA-A pushes target batches through the same encoder every step, and
    # source-only calibration risks under-covering target-domain neighbor counts, the same
    # failure mode that caused B1's original CUDA OOM incident).
    ...
    domain_disc = DomainDiscriminator(in_dim=KPCONV_FEAT_DIM, dropout=args.disc_dropout).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (src_batch, trgt_batch) in enumerate(zip(src_train_loader, trgt_iter)):
            ...
            # target species labels intentionally never read below (DA-A never uses target
            # labels, same discipline as A2/C2) -- only trgt_batch.species_labels being unused
            # matters here, so no separate variable is bound for it.
            ...
            dom_labels = torch.cat([
                torch.zeros(src_species.size(0), dtype=torch.long, device=device),
                torch.ones(trgt_batch.species_labels.size(0), dtype=torch.long, device=device),
            ], dim=0)
