"""
train_a2_pointnet2_da_a.py

Strategy table row A2: PointNet++ backbone, DA-A (adversarial domain
adaptation), ALL augmentation, L_cls + L_seg + L_dom + L_ent -- the
PointNet++-track counterpart of adapters/train_c2_dgcnn_da_a.py (row C2,
DGCNN). Deliberately mirrors that script almost line-for-line: same DA-A
protocol, same adversarial machinery (adapters/dann.py, reused
byte-for-byte, already fixed for the ramped-lambda_ent bug found and
corrected during C2 -- see that file's "REVISED 2026-09-11" section), same
Kendall({"cls","seg"}) combination -- only the backbone
(adapters/models_pointnet2.py::PointNet2_ClsSeg in place of
adapters/models.py::DGCNN_ClsSeg) differs, so cross-backbone comparisons
under the identical DA-A/ALL cell are attributable to architecture, not to
incidental training-loop differences.

Purpose of this row: per explicit user instruction, this directly tests
whether the pattern found on the DGCNN track (DA-A, and DA-S, both
underperforming the DA-0 baseline -- see step_notes/C2_DGCNN_DA_A.md,
step_notes/C3_DGCNN_DA_S.md, step_notes/C4_DGCNN_DA_A_LN.md) is
DGCNN-specific, or a general property of this dataset's source/target
label-prior mismatch (Crops3D 73% Maize vs. Pheno4D 62-63% Tomato) and
small-N training regime (263 source-train / 160 target-adapt samples).
Since `dann.py` is backbone-agnostic and reused unchanged, and PointNet2's
`forward` now also exposes `logits["feat"]` (added for this row -- see
adapters/models_pointnet2.py), any difference between A2 and C2's
qualitative behavior is attributable to backbone architecture, not to any
difference in the adversarial method itself.

Compares against A1 (train_a1_pointnet2_da0.py) the same way C2 compares
against C1: both use the identical `pheno4d_heldout_eval.csv` target test
set/protocol, so A1-vs-A2's gap is directly comparable to C1-vs-C2's gap.

Per explicit user instruction, gets the same full-trajectory analysis as
C2/C3/C4 from the start (not just the selected checkpoint) once results
land -- see step_notes/A2_PointNet2_DA_A.md.

Model selection: same protocol as A1/C1/C2 -- best checkpoint by lowest
source validation total loss (Kendall(cls,seg) on Crops3D val only).
L_dom/L_ent are intentionally excluded (see train_c2_dgcnn_da_a.py's
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Row A2: PointNet++, DA-A (adversarial), ALL augmentation, L_cls+L_seg+L_dom+L_ent")
    p.add_argument("--exp_name", type=str, default="A2_pointnet2_da_a")
...
def main():
    ...
    # Target adaptation pool: points only, ALL augmentation applied (same
    # as source, per the row's DA-A/ALL spec) -- labels loaded but never
    # used in the loss, same discipline as C2.
