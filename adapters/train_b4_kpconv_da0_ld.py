"""
train_b4_kpconv_da0_ld.py

Strategy table row B4: KPConv backbone, DA-0 (source-only, no adaptation), L-D augmentation
ONLY (RandomCrop3D + CoarseDropout3D, not the full ALL mix), L_cls + L_seg. Deliberately mirrors
adapters/train_b1_kpconv_da0.py line-for-line -- the DA-0 protocol, L_cls/L_seg spec, Kendall
combination, and eval procedure are all identical to B1; the ONLY change is
`augment_mode="ld_only"` on `src_train_set` (see adapters/dataset.py/scripts/augmentations.py's
`compose_pipeline_ld_only` docstring), matching how row C4 (DGCNN, DA-A, L-N) differed from C2 by
exactly one augmentation-pipeline swap and nothing else.

Per CLAUDE.md's strategy table, B4 is "deliberately unadapted -- confirms claimed density
robustness": unlike A4/C4 (which pair L-D/L-N with an adversarial method to isolate a named
weakness), B4 pairs L-D with NO adaptation method at all. This is intentional, not an oversight
-- Block B was specced to use B4, not a DA-S row, as its own density-robustness check (see
CLAUDE.md's Backbones section: KPConv's claimed strength is "density-robust due to grid-
subsampled neighborhoods, not fixed point count"). L-D (crop + dropout) directly perturbs local
point density/coverage, so B4 asks whether KPConv's encoder handles that perturbation gracefully
with nothing else helping it -- a purer test of the architecture's own claimed property than any
row with an adaptation method layered on top.

Given B1's already-mixed stability story (DGCNN-like collapse resistance, but the highest
continuous variance and most misleading selection signal of any DA-0 baseline in the project --
see step_notes/B1_KPConv_DA0.md), and B2/B3/B3b's finding that KPConv responds well specifically
to cooperative adaptation losses but not adversarial ones (step_notes/B3_KPConv_DA_D.md), B4
isolates a different question entirely: does the raw architecture (no adaptation signal of any
kind) hold up against a targeted density/coverage perturbation on its own? This directly tests
the architecture's claimed property in isolation, uncontaminated by any adaptation method's
effect.

Trains on Crops3D (source) only -- no target data or target loss of any kind, since DA-0 is the
"no adaptation" lower bound. Pheno4D (target) is used purely for held-out classification eval,
to measure the sensor-domain gap; its labels are never used for training. Target segmentation
eval is NOT wired here, same reason as B1/C1/A1.

L_cls = L_CE (species Tomato-vs-Maize, weighted per-class).
L_seg = L_wCE + lambda_lov * L_Lovasz, per species (Tomato num_classes=3, Maize num_classes=6).
Combined via learned Kendall uncertainty weighting, NOT a plain sum -- identical to every other
row.
"""
...
AUGMENT_MODE = "ld_only"  # the one, deliberate difference from train_b1_kpconv_da0.py
...
def parse_args():
    p = argparse.ArgumentParser(description="Row B4: KPConv, DA-0, L-D augmentation only, L_cls+L_seg")
    p.add_argument("--exp_name", type=str, default="B4_kpconv_da0_ld")
...
def evaluate_cls_only(model, loader, criterion, device):
    """Classification-only eval for Pheno4D (target) -- domain-gap indicator, same convention
    as every other row. `batch.labels` here are placeholder zeros (see kpconv_collate.py's
    make_kpconv_collate_fn_cls_only docstring) -- never read below."""
    model.eval()
...
def main():
    ...
    # source-only calibration is sufficient here, same as B1: DA-0 never passes target points
    ...
    for epoch in range(args.epochs):
        ...
        io.cprint(f"Eval(no-train) - Target {epoch}, acc: {trgt_acc:.4f}, "
                  ...
                  f"<- domain gap indicator (DA-0 baseline, L-D only, cls-only)")
