"""
train_a1_pointnet2_da0.py

Strategy table row A1: PointNet++ backbone, DA-0 (source-only, no
adaptation), ALL augmentation, L_cls + L_seg -- the PointNet++-track
counterpart of adapters/train_c1_dgcnn_da0.py (row C1, DGCNN). Deliberately
mirrors that script almost line-for-line: same DA-0 protocol, same L_cls/
L_seg spec, same Kendall uncertainty combination, same eval procedure --
only the backbone (adapters/models_pointnet2.py::PointNet2_ClsSeg in place
of adapters/models.py::DGCNN_ClsSeg) differs, so cross-backbone comparisons
under the identical DA-0/ALL cell are attributable to architecture, not to
incidental training-loop differences. See models_pointnet2.py's docstring
for how the PointNet++ reference repo (yanx27/Pointnet_Pointnet2_pytorch,
vendored as a sibling repo the same way as DefRec_and_PCM) was checked for
compatibility and composed into the same {"cls": ..., "seg_feat": ...}
interface.
...
gap; its labels are never used for training. Target segmentation eval is
NOT wired here, same reason as C1 (see that script's docstring).
...
(both classification-type per the strategy table), NOT a plain sum -- see
adapters/losses.py.
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Row A1: PointNet++, DA-0, ALL augmentation, L_cls+L_seg")
    p.add_argument("--exp_name", type=str, default="A1_pointnet2_da0_clsseg")
...
def evaluate_cls_only(model, loader, criterion, device):
    ...
    indicator, same convention as train_c1_dgcnn_da0.py. Target
...
def main():
    ...
    # the strategy table defines for L_seg, already validated on the DGCNN
    # track (row C1).
